/*******************************
 * agenda-adm.js (corrigido)
 * - render tabela
 * - filtros
 * - export CSV
 * - popover robusto (Editar / Mover / Cancelar)
...
const profissionais = [
  ...new Set(
    agendamentos
      .filter(a => a.profissional && a.profissional !== "Equipe" && a.cliente !== "--")
...
const servicos = [
  ...new Set(
    agendamentos
      .filter(a => a.servico && !/Intervalo|Reunião|Manutenção/i.test(a.servico))
      .map(a => a.servico)
  )
].sort();
...
function passaFiltro(ag) {
  if (!ag) return true;
  if (filtroProfissional && filtroProfissional.value && ag.profissional !== filtroProfissional.value) return false;
  if (filtroServico && filtroServico.value && ag.servico !== filtroServico.value) return false;
  return true;
}
...
if (filtroProfissional) filtroProfissional.addEventListener("change", renderTabela);
...
function openPopover(td, ev, pin = false) {
  ...
  if (status === "ocupado") {
    content.innerHTML = `
      ...
      <div style="opacity:.8">${td.dataset.profissional || ""} • ${td.dataset.duracao || ""}</div>
      <div style="font-size:.85rem;margin-top:6px;color:#444">${td.dataset.cliente || ""}</div>
    `;
  } else if (status === "bloqueado") {
    content.innerHTML = `<div><strong>${td.dataset.servico || "Bloqueado"}</strong></div>`;
  } else {
  ...
  const maxLeft = window.scrollX + docEl.clientWidth - popover.offsetWidth - 12;
  ...
  if (btnEditar) btnEditar.disabled = status === "livre" || status === "bloqueado";
  ...
  if (btnEditar) {
    btnEditar.onclick = () => {
      ...
      alert(`Editar (simulação) — ${popoverAnchor.dataset.servico || ""}`);
...
const popupNovoHorarioFechar = document.getElementById("popupNovoHorarioFechar");
const btnNovoHorario = document.getElementById("btnNovoHorario");
const popupNovoHorarioSalvar = document.getElementById("popupNovoHorarioSalvar"); // pode ser undefined no markup atual
...
if (btnNovoHorario && popupNovoHorario) {
  btnNovoHorario.addEventListener("click", (ev) => {
    ...
    const firstInput = popupNovoHorario.querySelector("input,select,textarea,button");
...
if (popupNovoHorarioFechar && popupNovoHorario) {
  popupNovoHorarioFechar.addEventListener("click", (ev) => {
    ev.preventDefault();
    if (popupNovoHorario.classList) popupNovoHorario.classList.remove("open");
...
if (popupNovoHorarioSalvar && popupNovoHorario) {
  popupNovoHorarioSalvar.addEventListener("click", (ev) => {
    ev.preventDefault();
    const data = popupNovoHorario.querySelector("#novo_horario_data")?.value || "";
    const hora = popupNovoHorario.querySelector("#novo_horario_hora")?.value || "";
    const cliente = popupNovoHorario.querySelector("#novo_horario_cliente")?.value || "";
    if (!data || !hora) {
      alert("Preencha a data e o horário.");
      return;
    }
    alert(`Salvo (simulação): ${data} ${hora} - ${cliente}`);
    // fecha
    if (popupNovoHorario.classList) popupNovoHorario.classList.remove("open");
    popupNovoHorario.style.display = "none";
    popupNovoHorario.setAttribute("aria-hidden", "true");
